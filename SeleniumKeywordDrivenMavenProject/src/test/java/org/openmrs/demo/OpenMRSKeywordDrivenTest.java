...
public class OpenMRSKeywordDrivenTest extends OpenMRSKeywordDrivenBaseTest {

	@Test(priority = 0, groups = { "SmokeTest", "SanityTest", "RegressionTest" }, dataProvider = "TestDataProvider")
	public void registerPatientTest(Map<String, String> testData) {
		...
		Reporter.log("<img src=\" " + Utilities.screenshotPath + "\">");
		registrationPage.enterName(testData.get("Name"));
		registrationPage.clikNextButton();
		registrationPage.selectGender(testData.get("Gender"));
		registrationPage.clikNextButton();
		registrationPage.setDateOfBorth(testData.get("DateOfBirth"));
		registrationPage.clikNextButton();
		...
		registrationPage.clikNextButton();
		registrationPage.clikNextButton();
		assertTrue(registrationPage.verifyRegistrationDetails(testData.get("Name"), testData.get("Gender"),
				testData.get("DateOfBirth"), testData.get("PhoneNumber")));
		Assert.assertEquals(true, registrationPage.verifyRegistrationDetails(testData.get("Name"),
				testData.get("Gender"), testData.get("DateOfBirth"), testData.get("PhoneNumber")),
